# How Matter Learns to Continue: v45 reader companion

This local delivery accompanies the **v45 PDFs** whose exact hashes are in
`evidence/v45-compatibility.json`. It preserves the v40 scientific payload,
the historical v34/v39 execution records, and the four v44 additions byte for
byte. Their older names and paths identify their actual provenance.
`bundle-manifest.json` keeps the inherited v40 compatibility field needed by
the unchanged verifier. The current delivery binding is the separate v45 record.

Extract the ZIP and open a terminal in `how-matter-learns-to-continue-v45-companion`. Install the requirements
using the inherited setup below. These commands print reports without changing
...
The first command runs the unchanged 35-command portable suite, including two
Chapter 3 training reproductions in memory. It checks frozen E3 arithmetic and
framework records. It does not run E3 instruments or neural frameworks. Current
v45 printed framework snippets are byte-equal to the inherited snippets, whose
original execution remains historical.

## Retained v44 lab and optional extensions

`orthogonal-checks-v44.py` is the unchanged **v44** standard-library lab.
Its nine groups automatically use the included frozen E3 evidence.
`deep-dive.md` is the unchanged v44 optional extension document, developing
horizon, episode-reversal, ideal infinite-walk, and matched-randomness examples.
These retain their original edition attribution. They are not new v45 results.

The two files in `evidence/v44/` preserve the original repository lab snapshot
and report, including their original paths, time, and v43 input archive.
`README-v44-original.md`, `evidence/v44-bundle-manifest-original.json`, and
`evidence/v44-companion-delivery-inherited.json` preserve the earlier delivery
context. The new v45 packaging run separately checks that these four additions
match the published v44 archive exactly and freshly reruns its nine-group lab.

## New v45 connection calculations

`connection-checks-v45.py` independently checks eleven mathematical groups
from the declared teaching models and reviewer examples. They cover optimizer
closure, five-label inference, path continuation, the bounded two-policy
comparison, dependence, and other explicitly scoped mathematical connections.
These checks do not imply that every optional connection was added to the book.

The optional `--e3` argument above also evaluates the P06 squared-error
decomposition on all thirty saved rows after verifying their pinned hash.
This is later arithmetic on exposed records, not a rerun of original training,
probe trajectories, or E3 instruments. It does not validate a repaired predictor
or establish a population error bound. With no `--e3` argument, the eleven
groups and general P06 identities run, while saved-row work is marked
`not_requested`.

`evidence/v45/connection-results-v45.json` retains the repository-run snapshot.
The fresh extraction results are recorded separately alongside the book in
`reviews/publication-v45/validation-companion-v45.json`. Its three suites,
isolated virtual environment, extracted-file hashes, and unchanged file set
are checked by the packager. None of these calculations is new natural evidence,
a human-reader study, or a completed physical maintenance mechanism.

## Edition binding and provenance

`evidence/v45-source-route-inventory.json` hashes the current master, all
manifest chapter/notes files, and recursively linked static TeX inputs.
`evidence/v45-source-manifest.json` retains the current manifest.
Complete sources and figure assets are not bundled. Original generators remain
excluded or marked as frozen provenance.

`evidence/v42-companion-replay-inherited.json` is the unchanged **v42** receipt
for the original v40 archive. Its original recorded interval is
2026-09-17T14:31:06.393155+00:00 to 2026-09-17T14:31:49.517087+00:00.
It is inherited evidence, not a fresh v45 replay of that old archive.
The fresh checks target this new v45 delivery and retain the v44 lab's identity.

Keep nested directories intact when extracting. This is a local reader
companion, not a complete figure-regeneration repository, accessible alternative
edition, or public release. Purchaser access still needs an established and
tested route.

## Retained setup, reader routes and historical evidence

The inherited instructions below retain their original edition attribution.
Their historical results are separate from the fresh v45 delivery checks.
