# How Matter Learns to Continue: v44 reader companion

This local delivery accompanies the two **v44 PDFs** whose exact hashes are in
`evidence/v44-compatibility.json`. It preserves the v40 scientific payload and
historical v34/v39 execution records. Their older paths identify provenance.
`bundle-manifest.json` retains the inherited v40 compatibility field required
by its unchanged verifier; the delivery binding is the separate v44 record.

Extract this ZIP and open a terminal in `how-matter-learns-to-continue-v44-companion`. The setup and reader commands
below use this directory. `python -B verify_companion.py` runs 35 portable
commands, including two Chapter 3 training reproductions in memory. It checks
frozen E3 arithmetic and framework records; it does not execute E3 instruments
or neural frameworks. The current v44 printed framework snippets were checked
byte-for-byte against the inherited snippets. Their fresh execution is pending.

## New v44 lab and optional extensions

From this extracted directory, run `python -B orthogonal-checks-v44.py`.
This independent, standard-library-only lab runs nine numerical groups and
prints JSON without writing files. It automatically finds the included frozen
E3 evidence. `python -B orthogonal-checks-v44.py --output ../v44-rerun.json`
saves a report outside the extracted tree. The destination's parent must exist.
The lab verifies the original artifact bytes and re-evaluates their arithmetic.
It does not regenerate historical E3 training or the original probe trajectories.

`deep-dive.md` develops the horizon, episode-reversal and ideal infinite-walk
extensions and the matched-randomness counterexample. These later conditional
calculations do not change the recorded original experiments. The new lab also
calculates the projected error and descriptor grouping on all 30 frozen E3 rows.

`evidence/v44/orthogonal-results-v44.json` and the adjacent lab report preserve
the repository run, including its original paths, time and v43 archive input.
The fresh default invocation in this v44 extraction is recorded separately in
`reviews/publication-v44/validation-companion-v44.json` alongside the book.
These records distinguish old scientific payload, new analysis and fresh replay.

## Edition binding and provenance

The TeX source and reader-route hash inventory is in
`evidence/v44-source-route-inventory.json`; it covers the current master,
manifest chapter/notes files and recursively linked static TeX inputs.
The v44 source manifest is included separately. Hashes bind this delivery to
those sources; the complete source tree and figure assets are not bundled.
Original generators remain excluded or marked as frozen provenance.
...
That evidence is inherited, not a fresh v44 replay of the original archive.
The new v44 delivery receives its own fresh 35-command replay in an isolated
extraction; its receipt accompanies the book as
`reviews/publication-v44/validation-companion-v44.json`.

This is the ZIP named in the v44 reading edition. Keep its nested directories
intact when extracting and running the reader commands. It is not a complete
figure-regeneration repository, accessible alternative edition, or public
release. Purchaser access still needs an established and tested route.

## Retained setup, reader routes and historical evidence

The following inherited instructions and reports retain their original edition
attribution. Fresh v44 delivery replay evidence is supplied alongside the book.
