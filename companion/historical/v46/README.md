# How Matter Learns to Continue: v46 reader companion

This local delivery accompanies the **v46 covered and interior PDFs** whose
exact hashes appear in `evidence/v46-compatibility.json`. It preserves the
frozen v40 scientific payload and the actual four v44 and two v45 scientific
additions byte for byte, under their original names. Historical execution
records remain historical; this package has its own fresh validation receipt.

## Start here

Open `where-to-return.html` (or `where-to-return.md`) for eight questions
leading back to worked examples. Put the extracted `how-matter-learns-to-continue-v46-companion` folder beside
`how-matter-learns-to-continue-v46.pdf` and/or
`how-matter-learns-to-continue-v46-interior.pdf`. The PDFs are supplied
separately, not duplicated in the ZIP. Links use named destinations; printed
and physical page numbers are also supplied for PDF viewers that ignore them.
The HTML guide also accepts a local PDF selection for other folder layouts.

Try `reader-exercise.html` before opening its separate answer key. It asks
whether one retained action preserves confidence about the calibration history.
`reader-pilot.html`, the two blind cards and the response form are prepared
materials for a six-task real-reader pilot. The facilitator file contains the
card mapping and scoring rubric. **No human readers have been tested here.**
The exercise and guide do not add pages to the book or modify its ending.

## Run the supplied checks

Extract the ZIP, open a terminal in `how-matter-learns-to-continue-v46-companion`, and install the requirements
using the inherited setup below. Each command prints a report without changing
...
The unchanged first command runs 35 portable commands, including two small
Chapter 3 training reproductions in memory. It checks frozen E3 arithmetic
and historical framework records; it does not run original E3 instruments or
original neural-framework training. Current printed framework snippets remain
byte-equal to the inherited snippets; their recorded execution is historical.

The unchanged **v44** lab runs nine numerical groups, automatically using the
included frozen E3 evidence. `deep-dive.md` and `evidence/v44/` retain
their original extension text, lab snapshot, report and provenance.

The unchanged **v45** lab runs eleven connection groups. The command above also
checks P06 error decomposition on all thirty saved rows after verifying their
pinned hash. This is saved-row arithmetic, not recreation of original probes,
training, trajectories or a newly validated repair. Omitting `--e3` runs the
eleven groups and general identities while marking saved-row work not requested.
The original v45 review identity remains recorded with this historical lab.

The new **v46** standard-library lab runs N01-N04: 128 constructed action cases,
decision versus conditional confidence, exact total expectation, and two
acceleration protocols. These are mathematical checks of teaching examples.
They do not constitute new natural evidence or observed reader performance.

## Edition binding and preservation

`evidence/v46-source-route-inventory.json` hashes the current master, every
manifest chapter and notes file, and recursively included static TeX inputs.
`evidence/v46-source-manifest.json` preserves the current manifest.
`reader-guide-bindings.json` binds question links to final aux labels and
the named destinations resolved separately in both final PDFs. Complete
sources and figure assets are excluded.

`README-v40-original.md`, `README-v44-original.md`,
`README-v45-original.md`, original manifests, and inherited receipts preserve
earlier context. The unchanged v40 verifier's compatibility field remains in
`bundle-manifest.json`; the current binding is the separate v46 record.
The v42 receipt records the old v40 replay from
2026-09-17T14:31:06.393155+00:00 to 2026-09-17T14:31:49.517087+00:00.
It has not been relabeled as a fresh v46 execution.

The v46 packager freshly executes all four suites in a clean extraction and
isolated virtual environment, compares saved-row and mathematical results,
checks unchanged extracted bytes/file set and final PDF/source hashes, and
creates the final ZIP exclusively. Its receipt is kept beside the book at
`reviews/publication-v46/validation-companion-v46.json`.

This is a local delivery. A public download or purchaser access route has not
been established. The book PDF's semantic accessibility, assistive-technology
reading order, printed proof, and physical apparatus formation/maintenance
are not certified by these checks.

## Retained setup, reader routes and historical evidence

The inherited instructions below keep their original edition attribution.
